...
var tryCatch = map[string]int{}
var myPokeMap = map[string]CatchedPoke{} 
...
func pokeCatch(url string) error {
    res, err := http.Get(url)
    if err != nil {return err}
    if res.StatusCode > 299 {fmt.Println("status code:", res.StatusCode)}
    defer res.Body.Close()
    
    myPoke := CatchedPoke{}
    decoder := json.NewDecoder(res.Body)
    err = decoder.Decode(&myPoke)
    if err != nil {return err}

    fmt.Println("Pokemon:", myPoke.Name, "Base EXP:", myPoke.BaseExperience)

    _, okk := tryCatch[myPoke.Name]
    if !okk {
        tryCatch[myPoke.Name] = myPoke.BaseExperience
    }
    tryToCatch(myPoke) 
...
func tryToCatch(myPoke CatchedPoke) error {        
        if tryCatch[myPoke.Name] > 50 {
            tryCatch[myPoke.Name] -= 50
            fmt.Printf("%v escaped!\n", myPoke.Name)
        } else if tryCatch[myPoke.Name] <= 50 {
            fmt.Printf("%v was caught!\n", myPoke.Name)
            myPokeMap[myPoke.Name] = myPoke
        }
        return nil
}
